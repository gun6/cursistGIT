...
<%@taglib prefix='c' uri='http://java.sun.com/jsp/jstl/core' %>
	...
	<nav>
		<ul>
			...
			<c:if test="${not empty mandje} && ${title ne 'reservatiemandje'}"><li><a>Reservatiemandje</a></li></c:if>
			<c:if test="${not empty mandje} && ${title ne 'bevestiging reservaties'}"><li><a>Bevestiging reservatie</a></li></c:if>
		</ul>
	</nav>
