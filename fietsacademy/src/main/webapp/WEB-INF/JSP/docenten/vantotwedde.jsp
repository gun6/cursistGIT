...
<html lang='nl'>
	...
	<body>
		...
		<form>
			...
			<label>Tot:<span>${fouten.tot}</span><input name='tot' value='${empty tot ? param.tot : tot}' type="number" min="0" step="0.01" required></label>
		...
		<c:if test="${not empty docenten}">
			...
			<c:if test="${empty laatstePagina}">
				...
				<a href='<c:out value="${volgendePaginaURL}"/>' title="volgede pagina" class='pagineren'>&rarr;</a>
			</c:if>
		</c:if>	
	</body>
</html>